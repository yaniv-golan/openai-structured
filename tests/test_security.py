...
from openai_structured.cli.errors import PathSecurityError
...
def test_security_manager_init(tmp_path: Path) -> None:
    ...
    manager = SecurityManager(base_dir, [allowed_dir])
...
def test_is_path_allowed(tmp_path: Path) -> None:
    ...
    base_file.touch()
    assert manager.is_path_allowed(base_file)
    
    # Test paths under allowed directory
    allowed_file = allowed_dir / "test.txt"
    allowed_file.touch()
    assert manager.is_path_allowed(allowed_file)
    
    # Test paths outside allowed directories
    outside_file = outside_dir / "test.txt"
    outside_file.touch()
    assert not manager.is_path_allowed(outside_file)
    
    # Test non-existent paths
    assert not manager.is_path_allowed(base_dir / "nonexistent.txt")
...
def test_add_allowed_dir(tmp_path: Path) -> None:
    ...
    manager = SecurityManager(base_dir)
    assert not manager.is_path_allowed(new_dir)
    
    manager.add_allowed_dir(new_dir)
    assert manager.is_path_allowed(new_dir)
...
def test_add_allowed_dirs_from_file(tmp_path: Path) -> None:
    ...
    manager = SecurityManager(base_dir)
    manager.add_allowed_dirs_from_file(allowed_file)
    
    assert manager.is_path_allowed(dir1)
    assert manager.is_path_allowed(dir2)
...
def test_validate_path(tmp_path: Path) -> None:
    ...
    valid_file.touch()
    assert manager.validate_path(valid_file) == valid_file.resolve()
    
    # Test invalid path
    invalid_file = outside_dir / "test.txt"
    invalid_file.touch()
    with pytest.raises(PathSecurityError):
        manager.validate_path(invalid_file)
...
def test_is_allowed_file(tmp_path: Path) -> None:
    ...
    test_file.touch()
    assert manager.is_allowed_file(test_file)
    
    # Test directory
    test_dir = base_dir / "test_dir"
    test_dir.mkdir()
    assert not manager.is_allowed_file(test_dir)
    
    # Test non-existent file
    assert not manager.is_allowed_file(base_dir / "nonexistent.txt")
