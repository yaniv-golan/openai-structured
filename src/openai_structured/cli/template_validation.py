...
from .file_utils import FileInfo, TemplateValue
...
def validate_template_placeholders(task_template: str, template_context: Dict[str, Any], env: Optional[Environment] = None) -> None:
    ...
    logger.debug("Args: task_template=%s, template_context=%s", task_template, {k: type(v).__name__ for k, v in template_context.items()})
    ...
    try:
        ...
        builtin_vars = {
            ...
            'loop', 'super', 'self', 'varargs', 'kwargs', 'items',
            # String filters
            'upper', 'lower', 'title', 'capitalize', 'trim',
            'strip', 'lstrip', 'rstrip', 'center', 'ljust', 'rjust',
            'wordcount', 'truncate', 'striptags',
            # List filters
            'first', 'last', 'length', 'max', 'min', 'sum',
            'sort', 'unique', 'reverse', 'reject', 'select',
            'map', 'join', 'count',
            # Type conversion
            'abs', 'round', 'int', 'float', 'string', 'list',
            'bool', 'batch', 'slice', 'attr',
            # Common filters
            'default', 'replace', 'safe', 'urlencode', 'indent',
            'format', 'escape', 'e', 'nl2br', 'urlize',
            # Dictionary operations
            'items', 'keys', 'values', 'dictsort',
            # Math operations
            'round', 'ceil', 'floor', 'abs', 'max', 'min',
            # Date/time
            'now', 'strftime', 'strptime', 'datetimeformat',
        }
        ...
        missing = {
            name for name in found_vars 
            if name not in available_vars 
            and name not in builtin_vars 
            and name not in loop_vars
            and name != 'stdin'  # Special case for stdin which may be added dynamically
        }
        ...
        logger.debug("Before create_validation_context - available_vars type: %s, value: %s", type(available_vars), available_vars)
        logger.debug("Before create_validation_context - template_context type: %s, value: %s", type(template_context), template_context)
        
        # 3) Create validation context with proxy objects
        logger.debug("Creating validation context with template_context: %s", template_context)
