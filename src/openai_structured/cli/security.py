...
def is_temp_file(path: str) -> bool:
    ...
    for temp_dir in temp_dirs:
        ...
        if len(abs_path_parts) >= len(temp_dir_parts) and \
           all(a == b for a, b in zip(abs_path_parts[:len(temp_dir_parts)], temp_dir_parts)):
            return True
...
class SecurityManager(SecurityManagerProtocol):
    ...
    def __init__(self, base_dir: Optional[str] = None, allowed_dirs: Optional[List[str]] = None):
    ...
    def add_allowed_dirs_from_file(self, file_path: str) -> None:
        ...
        try:
            self.validate_path(str(real_path), purpose="read allowed directories")
        except PathSecurityError:
            raise PathSecurityError(f"Cannot read allowed directories file outside allowed paths: {file_path}")
    ...
    def validate_path(self, path: str, purpose: str = "access") -> Path:
        ...
        if not self.is_path_allowed(str(real_path)):
            raise PathSecurityError(f"Path {purpose} denied: {path} is outside allowed directories")
