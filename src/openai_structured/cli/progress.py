...
class ProgressContext:
        ...
        if self.enabled and self._progress:
            if exc_type is None:
                ...
                if self._task_id is not None:
                    self._progress.update(self._task_id, completed=self.total or self.current)
            self._progress.stop()
    ...
    def step(self, description: Optional[str] = None) -> Iterator[None]:
        ...
        """
        if description and self.enabled and self._progress and self._task_id is not None:
            old_description = self._progress.tasks[self._task_id].description
            self._progress.update(self._task_id, description=f"{old_description} • {description}")
        try:
            yield
        finally:
            if self.enabled and self._progress and self._task_id is not None:
                self.update()
                if description:
                    self._progress.update(self._task_id, description=self.description)
